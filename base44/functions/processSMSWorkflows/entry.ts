...
Deno.serve(async (req) => {
    try {
        ...
        console.log('🔄 Processing SMS workflow executions via SignalHouse...');

        const now = new Date();
        const executions = await base44.asServiceRole.entities.SMSWorkflowExecution.filter({ status: 'active' });
        const dueExecutions = executions.filter(e => e.next_message_date && new Date(e.next_message_date) <= now);
        ...
        for (const execution of dueExecutions) {
            try {
                const workflows = await base44.asServiceRole.entities.SMSWorkflow.filter({ id: execution.workflow_id });
                ...
                console.log(`📤 Sending step ${currentStep} to ${execution.phone_number} via SignalHouse`);
                ...
                results.push({ execution_id: execution.id, phone_number: execution.phone_number, step: currentStep, status: 'sent' });

            } catch (error) {
                console.error(`Error processing execution ${execution.id}:`, error);
                await base44.asServiceRole.entities.SMSWorkflowExecution.update(execution.id, { status: 'failed' });
                results.push({ execution_id: execution.id, error: error.message });
            }
        }

        return Response.json({ success: true, processed: dueExecutions.length, results });
