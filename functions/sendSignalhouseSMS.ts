...
Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log('[sendSignalhouseSMS] body keys:', Object.keys(body));
        ...
        console.log('[sendSignalhouseSMS] authToken len:', authToken.length, 'apiKey len:', apiKey.length, 'from:', rawFrom);
        ...
        console.log('[sendSignalhouseSMS] payload:', JSON.stringify({
            from: payload.from,
            to: payload.to,
            apiKeyLen: apiKey.length,
            bodyLen: finalMessage.length
        }));
        ...
        console.log('[sendSignalhouseSMS] status:', response.status, 'body:', responseText.substring(0, 500));

        let data;
        try { data = JSON.parse(responseText); } catch (_) {
            return Response.json({ error: 'Non-JSON from SignalHouse', raw: responseText.substring(0, 300) }, { status: 500 });
        }
        ...
        console.error('[sendSignalhouseSMS] error:', response.status, JSON.stringify(data));
        return Response.json({
            success: false,
        ...
        console.error('[sendSignalhouseSMS] exception:', err.message);
